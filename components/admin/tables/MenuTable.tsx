...
export function MenuTable({ items, sort, order }: MenuTableProps) {
  ...
  function handleToggleAvailability(item: MenuItem) {
    startTransition(async () => {
      ...
      const result = await toggleAvailability(item.slug);
      if (result.success) {
  ...
  function handleToggleFeatured(item: MenuItem) {
    startTransition(async () => {
      ...
      const result = await toggleFeatured(item.slug);
      if (result.success) {
  ...
  async function handleDelete() {
    ...
    const result = await deleteMenuItem(deleteTarget.slug);
    if (result.success) {
  ...
  return (
    <>
          ...
          <tbody className="divide-y divide-green-soft/10">
            {optimisticItems.map((item) => (
              <tr key={item.id} className={isPending ? "opacity-80" : ""}>
                ...
                <td className="px-4 py-3">
                  <TableActions
                    editHref={`/admin/menu/${item.slug}/edit`}
      ...
      <DeleteConfirmDialog
        ...
        description={`This will permanently delete "${deleteTarget?.name}" and remove its image from storage.`}
